...
total_calories = sum([int(data[3]) for data in match])
days = total_calories // 2000

print(f"You have food to last you for: {days} days!")
if days > 0:
    for product_info in match:
        name, date, nutrition = product_info[1], product_info[2], product_info[3]
...
class Regex:
    ...
    def regex_action(self):
        match = re.findall(self.pattern, self.string)
        return match


class Research:
    __pattern = r'(\||\#)([A-Za-z\s]+)\1(\d{2}\/\d{2}\/\d{2})\1(\d+)\1'

    def __init__(self, data):
        self.data = data

    def find_product(self):
        packed_food = Regex(Research.__pattern, self.data).regex_action()
        return packed_food

    def find_calories(self):
        total_calories = sum([int(data[3]) for data in self.find_product()])
        return total_calories

    def calculate_days(self):
        days = self.find_calories() // 2000
        return days

    def create_report(self):
        days = self.calculate_days()
        report = f"You have food to last you for: {days} days!"
        if days > 0:
            for product_info in self.find_product():
                name, date, nutrition = product_info[1], product_info[2], product_info[3]
                report += f"\nItem: {name}, Best before: {date}, Nutrition: {nutrition}"
        return report
...
class Main:
    ...
    def search(self):
        message = Research(self.some_input).create_report()
        self.log = message
